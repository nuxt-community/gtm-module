...
export default function (ctx, inject) {
  _initialized = Object.assign(_initialized, {
    <%= (options.autoInit && options.id) ? ` '${options.id}': true ` : null %>
  })
  ctx.$gtm = process.client ? gtmClient(ctx, _initialized) : gtmServer(ctx, _initialized)
  inject('gtm', ctx.$gtm)
